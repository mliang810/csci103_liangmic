...
int binsearch(int target, int *data, int start, int end)
{
    ...
    //search from middle either direction, depending if the target is bigger or smaller 
    if(start+1 < end){ //to make sure you dont keep dividing into infinity
        ...
        if(data[midpoint] == target){
            ...
            binsearch(target, data, midpoint, end);
        }

        else if(data[midpoint] > target){
            binsearch(target, data, start, midpoint);
        } 
    }
...
void sort(int *data, int len)
{
    ...
    //max and other value swap positions
    int temp;
    temp = data[len-1]; 
    data[len-1] = data[spot]; //max gets put into where it should be
    data[spot] = temp; //put whatevers in the last slot into the old max position
    
    //recursion, keep swapping until completely in order/end of array
    if(len==1){ 
        return;
    }
    else{
