...
{
  int sum = 0;
  for(int i=0; i < len; i++){ //changed <= to <, m1, out of bounds
    sum += data[i];
  }
  return sum;
}
...
char* test2_reverse(char* word)
{
  ...
  otherword[len] = '\0'; //took out +1 in [len] so it doesnt go out of bounds
  /*delete [] word;*/ //gets deleted later, so removed it
  return otherword;
}
